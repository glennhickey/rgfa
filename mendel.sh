...
# run rtg
rtg mendelian -i mendel_tmp_${NAME} -t ${SDF} --pedigree ped_tmp_${NAME}.ped > ${VCF::-6}.mendel

rm -f mendel_tmp_${NAME} mendel_tmp_${NAME}.tbi

# make a phony heterozygous
bcftools view $VCF -i "QUAL>${MIN_QUAL} && FILTER=\"PASS\""  | sed -e 's#0/0#0/1#g' -e 's#1/1#0/1#g' -e 's#2/2#0/2#g' | bgzip --threads 2 > mendel_het_tmp_${NAME}
tabix -fp vcf mendel_het_tmp_${NAME}

# run rtg
rtg mendelian -i mendel_het_tmp_${NAME} -t ${SDF} --pedigree ped_tmp_${NAME}.ped > ${VCF::-6}.mendel.het
